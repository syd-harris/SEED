...
void loop() {
  ...
  if (docounting) {
    ...
    counter = millis() - timeStart;
     if (counter > 1000)
     {
      velocityRobotLinearSP = .65;
    }
    if (counter >= 2000) {
      Serial.println("Finished");
      velocityRobotLinearSP = 0;
      docounting = false;
...
//    }
    ...
    counter = 0;
     ...
     roePID_u = roePID_Kp*velocityRobotLinear + roePID_Ki*roePID_I + roePID_Kd*roePID_D;
     ...
     if (abs(velocityRobotLinearSP) < 0.0001){
      speed1 = 0;
      speed2 = 0;
     }
     else{
        speed1 = abs(roePID_u /2);
        speed2 = abs(roePID_u /2);
     }
...
//
  ...
  Serial.print("\t");
  Serial.print(velocityRobotLinear);
  Serial.println("");
//
